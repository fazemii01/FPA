...
def main():
    ...
    # 1. Rename the default .page selector to .report-page in the styles
    # We target:
    #         .page {
    #             width: 196mm;
    ...
    #         }
    # Note the double braces in the original template
    old_page_style = r'\.page\s*\{\{\s*width:\s*196mm;\s*page-break-after:\s*always;\s*position:\s*relative;\s*background-color:\s*#ffffff;\s*border:\s*6px\s*solid\s*#1B365D;\s*\}\}'
    new_page_style = """.report-page {{
            width: 196mm;
            page-break-after: always;
            position: relative;
            background-color: #ffffff;
    ...
    content = re.sub(old_page_style, new_page_style, content)
    
    # 2. Rename all <div class="page"> to <div class="report-page"> for pages 2 to 10
    # Pages 2 to 10 all start with <div class="page"> and are followed by <div class="page-inner">
    content = content.replace('<div class="page">', '<div class="report-page">')
    
    # 3. Read the clean base64 string
    b64_path = "e:\\my project\\ALLIA\\FPA\\backend\\original_b64.txt"
    with open(b64_path, "r", encoding="utf-8") as f:
        clean_b64 = f.read().strip()
        
    # 4. User's exact CSS style block for the cover page
    # Note: we use double curly braces {{ }} for formatting compatibility in Python.
    user_styles = """
        .page {{
            position: relative;
            width: 210mm;
    ...
    # 5. User's exact HTML cover page (using `<div class="page">` to match user exactly)
    user_html = f"""
    <!-- Page 1: COVER -->
    <div class="page">
        <!-- decorations -->
    ...
    # We target the old cover style block which starts with `.page-cover {{`
    style_pattern = r'\.page-cover\s*\{\{.*?\n\s*\}\}'
    
    match = re.search(style_pattern, content, re.DOTALL)
    if match:
        print("Found cover style block, replacing with user's .page cover style...")
        # Replace the cover style block with the user styles (using .page)
        new_content = re.sub(style_pattern, user_styles.strip(), content, flags=re.DOTALL)
    else:
        print("Style block not found or already replaced. Let's do a fallback search for the user style block.")
        # if .page-cover was already replaced, we can match .page {{ position: relative; ... }}
        fallback_style_pattern = r'\.page\s*\{\{\s*position:\s*relative;\s*width:\s*210mm;\s*height:\s*297mm;.*?\n\s*\}\}'
        match_fb = re.search(fallback_pattern if 'fallback_pattern' in locals() else fallback_style_pattern, content, re.DOTALL)
        if match_fb:
            print("Already has .page cover style.")
            new_content = content
        else:
    ...
    if match_html:
        print("HTML cover block match found, replacing...")
        final_content = re.sub(html_pattern, user_html.strip() + "\n\n    <!-- Page 2: MULTIPLE INTELLIGENCE -->", new_content, flags=re.DOTALL)
        with open(file_path, "w", encoding="utf-8") as f:
            f.write(final_content)
        print("Successfully updated html_generator.py to use exact cover styles and HTML!")
    else:
